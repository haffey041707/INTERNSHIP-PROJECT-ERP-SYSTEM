...
export default async function NewStudentPage() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h1 className="text-2xl font-extrabold text-slate-900 mt-2 mb-1">{terms.addLearner}</h1>
      ...
      <form action={action} className="space-y-4 bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <label className="block sm:col-span-2">
            <span className="text-sm text-slate-600">{terms.section}</span>
        ...
        <div className="flex flex-col gap-3 pt-2 sm:flex-row">
          <button className="rounded-lg bg-brand-600 px-4 py-2 text-sm text-white">Save {terms.learner}</button>
