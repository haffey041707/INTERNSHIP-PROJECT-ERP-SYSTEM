    ...
    <div className="space-y-6">
      <div className="premium-home-hero rounded-2xl p-5 text-white">
        ...
        <div className="mt-4">
          ...
          <p className="mt-2 text-sm text-white/75">{klass.sections.length} sections · {totalStudents} students · {totalCapacity} seats</p>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger">
        <Stat icon={<Users size={18} />} label="Students" value={String(totalStudents)} accent />
        <Stat icon={<CalendarDays size={18} />} label="Sections" value={String(klass.sections.length)} />
        <Stat icon={<FileText size={18} />} label="Exams" value={String(klass.sections.reduce((sum, section) => sum + section.exams.length, 0))} />
            ...
            {klass.sections.map((section) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-slate-500">{section._count.students} / {section.capacity} students</p>
                </Link>
              );
            })}
            {klass.sections.length === 0 && <Empty text="No sections created yet." />}
          </div>
          <form action={createSection} className="mt-4 space-y-2 border-t border-slate-200 pt-4">
            <input type="hidden" name="classId" value={klass.id} />
            <label className="block">
              <span className="text-xs text-slate-600">Section name</span>
              <input name="name" placeholder="e.g. 1-A" required
            ...
            <button className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-brand-600 hover:bg-slate-50">Add section</button>
          </form>
        </section>

        <section className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm">
          <h2 className="font-semibold text-slate-900">{selected ? selected.name : 'No section'}</h2>
          {selected ? (
            ...
            <Empty text="No sections created yet." />
          )}
        </section>
      </div>
    </div>
