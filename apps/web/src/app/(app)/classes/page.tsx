...
export default async function ClassesPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 mb-1">Classes &amp; Sections</h1>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
        <form action={createClassWithSection} className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <div>
              <h2 className="font-semibold text-slate-900">Create a class &amp; section</h2>
              <p className="text-xs text-slate-500">Adds the class and its first section together.</p>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field name="name" label="Class name" placeholder="Grade 1" required />
            <Field name="grade" label="Grade code" placeholder="1" />
            <Field name="sectionName" label="First section" placeholder="1-A" required />
            <Field name="capacity" label="Capacity" type="number" placeholder="40" />
          </div>
          <button className="mt-4 rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white">Create class &amp; section</button>
        </form>

        <div className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm">
          <h2 className="font-semibold text-slate-900">Structure summary</h2>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <Summary label="Classes" value={String(classes.length)} />
            <Summary label="Sections" value={String(totalSections)} />
          </div>
          <p className="mt-4 rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-500">
            Create at least one class and one section to complete the setup item.
          </p>
        </div>
      </section>
          ...
          <div key={c.id} className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500">{c.sections.length} sections</p>
              </div>
              <Link href={`/classes/${c.id}`} className="text-xs font-medium text-brand-600">Open class</Link>
            </div>
            <div className="flex flex-wrap gap-3">
              {c.sections.map((s) => (
                <Link key={s.id} href={`/classes/${c.id}?section=${s.id}`} className="px-4 py-3 rounded-lg bg-slate-50 border border-slate-200 hover:border-brand-400">
                  ...
                  <p className="text-xs text-slate-500">{s._count.students} / {s.capacity} students</p>
                </Link>
              ))}
              {c.sections.length === 0 && (
                <p className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-500">No sections yet.</p>
              )}
            </div>
            <form action={createSection} className="mt-4 grid gap-2 border-t border-slate-200 pt-4 sm:grid-cols-[1fr_120px_auto]">
              ...
              <input name="capacity" type="number" min={1} placeholder="40"
                className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-brand-500" />
              <button className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-brand-600 hover:bg-slate-50">Add section</button>
            </form>
          </div>
        ))}
        {classes.length === 0 && (
          <div className="rounded-xl border border-slate-200 bg-white p-6 text-center shadow-sm">
            <p className="font-medium text-slate-900">No classes yet.</p>
            <p className="mt-1 text-sm text-slate-500">Use the form above to create your first class and section.</p>
          </div>
        )}
      </div>
    </div>
